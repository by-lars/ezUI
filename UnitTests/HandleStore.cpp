...
#include "Utility/HandleStore.h"

using namespace Microsoft::VisualStudio::CppUnitTestFramework;

namespace DataStructures
{
	EZ_MAKE_STRONG_HANDLE(IntHandle);

	TEST_CLASS(HandleStore)
	{
	public:
		TEST_METHOD(StoreAndRetrieve) {
			ez::HandleStore<IntHandle, int> store;
			IntHandle a = store.PushBack(1);
			IntHandle b = store.PushBack(2);
			IntHandle c = store.PushBack(3);

			Assert::AreEqual(1, store.Get(a), L"Retrieved value not equal to stored value");
			Assert::AreEqual(2, store.Get(b), L"Retrieved value not equal to stored value");
			Assert::AreEqual(3, store.Get(c), L"Retrieved value not equal to stored value");
		}
		
		TEST_METHOD(StoreAndDelete) {
			ez::HandleStore<IntHandle, int> store;
			IntHandle a = store.PushBack(1);
			IntHandle b = store.PushBack(2);
			IntHandle c = store.PushBack(3);
